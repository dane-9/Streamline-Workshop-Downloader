...
class BrowserView(QMainWindow):
    ...
    window_system_move_trigger = QtCore.Signal()
    window_system_resize_trigger = QtCore.Signal(str)
    ...
    def __init__(self, window):
        ...
        self.window_system_move_trigger.connect(self.on_window_system_move)
        self.window_system_resize_trigger.connect(self.on_window_system_resize)
    ...
    def on_window_system_move(self):
        handle = self.windowHandle()
        if handle is None or not hasattr(handle, 'startSystemMove'):
            logger.warning('This Qt version does not support startSystemMove')
            return

        if not handle.startSystemMove():
            logger.warning('The window system rejected the interactive move request')

    def on_window_system_resize(self, edge):
        handle = self.windowHandle()
        if handle is None or not hasattr(handle, 'startSystemResize'):
            logger.warning('This Qt version does not support startSystemResize')
            return
        ...
        edge_names = {
            ...
            'west': ('LeftEdge',),
            'south': ('BottomEdge',),
            'southeast': ('BottomEdge', 'RightEdge'),
            'southwest': ('BottomEdge', 'LeftEdge'),
        }
        names = edge_names.get(edge)
        if names is None:
            logger.warning('Unsupported system resize edge: %s', edge)
            return

        qt_edges = getattr(QtCore.Qt, names[0])
        for name in names[1:]:
            qt_edges |= getattr(QtCore.Qt, name)

        if not handle.startSystemResize(qt_edges):
            logger.warning('The window system rejected the interactive resize request')
    ...
    def start_system_move(self):
        self.window_system_move_trigger.emit()

    def start_system_resize(self, edge):
        self.window_system_resize_trigger.emit(edge)
...
def start_system_move(uid):
    i = BrowserView.instances.get(uid)
    if i:
        i.start_system_move()


def start_system_resize(edge, uid):
    i = BrowserView.instances.get(uid)
    if i:
        i.start_system_resize(edge)
