...
class Window:
    ...
    def start_system_move(self) -> None:
        """Start a window-system-managed interactive move operation."""
        self.gui.start_system_move(self.uid)

    @_shown_call
    def start_system_resize(self, edge: str) -> None:
        """
        Start a window-system-managed interactive resize operation.

        ``edge`` is one of east, west, south, southeast, or southwest.
        """
        self.gui.start_system_resize(edge, self.uid)
